...
CREATE TABLE `inventario360_usuario` (
  ...
  `contrasenia` varchar(255) NOT NULL,
    `estado` ENUM('activo', 'inactivo', 'eliminado') NOT NULL DEFAULT 'activo', -- ¡Esta es la línea que debes agregar!
...
CREATE TABLE `inventario360_producto` (
  ...
  `fecha_registro` date NOT NULL DEFAULT CURRENT_DATE, -- Solución al error: DEFAULT CURRENT_DATE
...
CREATE TABLE `inventario360_registro_actividad` (
  ...
  CONSTRAINT `fk_registro_actividad_usuario1` FOREIGN KEY (`usuario_idusuario`) REFERENCES `inventario360_usuario` (`idusuario`) ON DELETE NO ACTION ON UPDATE NO ACTION
...
CREATE TABLE `inventario360_movimientos` (
  ...
  CONSTRAINT `fk_movimientos_usuario` FOREIGN KEY (`usuario_id`) REFERENCES `inventario360_usuario` (`idusuario`),
...
('acceso_view_gestor_usuarios', 'permiso acceso gestor usuarios', 'activo');
